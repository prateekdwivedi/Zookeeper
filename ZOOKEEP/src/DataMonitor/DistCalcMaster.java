    ...
    // As per specification we are creating ZNode through command line argument		
	 ...
	 // Method to close the object of ZooKeeper
	   ...
	   public static void main (String[] args )throws Exception{
		...
		// We are handling invalid input format
		if(!command.contains("@")){	
        ...
        if(stat != null) {
           ...
           // To create priority queue, which has path of the create node 
           Stat stat1 = createGroup.znode_exists("/"+znodes[4]+"/queue-"+priority);
           if(stat1 != null)priority--;
           createGroup.create(znodes[4]+"/queue-"+priority , commands[0], 0);
           priority--;
                }
		}
		catch(Exception e){
